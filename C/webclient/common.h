...
int main() {
   ...
   char                recvline[MAXLINE];
	...
	if (argc != 2 ) 
	   err_n_die("usage %s <server addres>\n");
	...
	if (inet_pton(AF_INET, argv[1], &servaddr.sinaddr) <= 0))
	  err_n_die("inet_pton error for %s", argv[1]); 

	// connect to the server
	if (connect(sockfd, (SA *) &servaddr, sizeof(servaddra) < 0 )
...
void err_n_die(const char *fmt, ...) {
   ...
   va_start(ap, fmt);
   vfprintf(stdoit,fmt,ap);
   fprintf(stdout,"\n");
   fflush(stdout);

   //
   if ( errno_save != 0 ) {
      fprintf(stdout,"(errno = %d) : %s\n"i,errno_save,stderror(errno_save));
	  fprintf(stdout, "\n");
	  fflush(stdout) ;
   }
   va_end(ap);
